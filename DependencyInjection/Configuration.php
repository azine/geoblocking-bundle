	        	...
	        	->arrayNode('countries')->info("only whitelist or blacklist can contain values.")->addDefaultsIfNotSet()
		        	->children()
		        		->variableNode('whitelist')
			        		->cannotBeEmpty()->info("at least the route for the access-denied-error-page must be given")
		        			->defaultValue(array('fos_user_security_login', 'fos_user_security_login_check', 'fos_user_security_logout'))->end()
